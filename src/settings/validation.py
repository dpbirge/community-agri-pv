...
def validate_scenario(scenario_path):
    ...
    required = ["scenario", "simulation", "energy_system", "water_system", "food_processing_system", "community_structure", "economics"]
    for section in required:
        if section not in scenario:
            errors.append(f"Missing required section: {section}")

    # Check energy system sections
    energy_infra = scenario.get("energy_system", {})
    if energy_infra:
        required_energy = ["pv", "wind", "battery", "backup_generator"]
        for section in required_energy:
            if section not in energy_infra:
                errors.append(f"Missing required energy_system section: {section}")
    ...
    # Check water system sections
    water_infra = scenario.get("water_system", {})
    if water_infra:
        ...
        for section in required_water:
            if section not in water_infra:
                errors.append(f"Missing required water_system section: {section}")
    ...
    # Check food processing system sections
    food_infra = scenario.get("food_processing_system", {})
    if food_infra:
        required_food = ["fresh_food_packaging", "drying", "canning", "packaging"]
        for section in required_food:
            if section not in food_infra:
                errors.append(f"Missing required food_processing_system section: {section}")
    ...
    if "water_system" in scenario:
        water = scenario["water_system"]
